...
import { LanguageClientOptions, ExecuteCommandParams, ExecuteCommandRequest } from "vscode-languageclient"
import { LanguageClient, State as ClientState, StreamInfo } from "vscode-languageclient/node"
...
function newClient(cmd: string, location: string, commandPrefix: string) {
  ...
  const clientOptions: LanguageClientOptions = {
	...
	{ scheme: 'file', language: 'tfvars', pattern: `${f.uri.fsPath}/**/*` }],
    workspaceFolder: f,
  ...
  return new LanguageClient(`languageServer/${location}`, `Language Server: ${location}`, serverOptions, clientOptions)
}
...
async function pathToBinary(): Promise<string> {
  if (!_pathToBinaryPromise) {
    ...
    if (command) {
      // Skip install/upgrade if user has set custom binary path
...
async function rootModulesCommand(languageClient: terraformLanguageClient, documentUri: string): Promise<any> {
  const requestParams: ExecuteCommandParams = {
    command: `${languageClient.commandPrefix}.terraform-ls.rootmodules`,
  ...
  if (vscode.window.activeTextEditor) {
    ...
    if (modules.rootModules.length > 1) {
      const selected = await vscode.window.showQuickPick(
        modules.rootModules.map((m) => m.uri),
        { canPickMany: false }
      )
